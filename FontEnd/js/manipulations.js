...
function loadEventsOnScreen(screen, arrayInfo) {
    ...
    arrayInfo.map(function (info, i) {
        ...
        var stringAddedHome = '<div class="card-event added-id-' + info['id'] + '"><div class="card-top"><div>' + info['title'] + '</div></div><div class="detail-divisor"></div><div class="description"><b>Descrição: </b>' + info['description'].substring(0, 20) + '...<p><b class="author-event">Criador: ' + info['author'] + '</b></p></div><div class="detail-divisor"></div><div class="informations-hour-date">' + datetime + '</div><div class="options-card-event"><a href="#event" class="more-option-card-event default-event" rel="' + info['id'] + '"><div>Saiba mais</div></a><div class="more-option-card-event default-event" rel="' + info['id'] + '">Participar</div></div></div>';
